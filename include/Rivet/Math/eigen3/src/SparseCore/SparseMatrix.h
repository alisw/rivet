...
/** \ingroup SparseCore_Module
  ...
  *          whereas it is now (starting from %Eigen 3.3) deprecated and always defined as RivetEigen::Index.
  ...
  : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _StorageIndex> >
{
  ...
  public:
    ...
    /** Preallocates \a reserveSize[\c j] non zeros for each column (resp. row) \c j.
      ...
      * Typical choices include std::vector<int>, RivetEigen::VectorXi, RivetEigen::VectorXi::Constant, etc.
      */
  ...
  public:
    ...
    inline void setIdentity()
    {
      ...
      RivetEigen::Map<IndexVector>(this->m_data.indexPtr(), rows()).setLinSpaced(0, StorageIndex(rows()-1));
      RivetEigen::Map<ScalarVector>(this->m_data.valuePtr(), rows()).setOnes();
      RivetEigen::Map<IndexVector>(this->m_outerIndex, rows()+1).setLinSpaced(0, StorageIndex(rows()));
      std::free(m_innerNonZeros);
      m_innerNonZeros = 0;
    }
...
protected:
    ...
    void assignDiagonal(const DiagXpr diagXpr, const Func& assignFunc)
    {
      ...
      if(m_data.size()==0 || overwrite)
      {
        ...
        RivetEigen::Map<ArrayXI>(this->innerIndexPtr(), n).setLinSpaced(0,StorageIndex(n)-1);
        RivetEigen::Map<ArrayXI>(this->outerIndexPtr(), n+1).setLinSpaced(0,StorageIndex(n));
        RivetEigen::Map<Array<Scalar,Dynamic,1> > values = this->coeffs();
        values.setZero();
...
/** Fill the matrix \c *this with the list of \em triplets defined by the iterator range \a begin - \a end.
  ...
  * \endcode
  * See for instance the RivetEigen::Triplet template class.
  *
...
EIGEN_DONT_INLINE SparseMatrix<Scalar,_Options,_StorageIndex>& SparseMatrix<Scalar,_Options,_StorageIndex>::operator=(const SparseMatrixBase<OtherDerived>& other)
{
  ...
  if (needToTranspose)
  {
    ...
    RivetEigen::Map<IndexVector> (dest.m_outerIndex,dest.outerSize()).setZero();
