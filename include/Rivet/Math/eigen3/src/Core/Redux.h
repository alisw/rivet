...
{
  return derived().redux(RivetEigen::internal::scalar_min_op<Scalar,Scalar, NaNPropagation>());
}
...
{
  return derived().redux(RivetEigen::internal::scalar_max_op<Scalar,Scalar, NaNPropagation>());
}
...
DenseBase<Derived>::sum() const
{
  if(SizeAtCompileTime==0 || (SizeAtCompileTime==Dynamic && size()==0))
    return Scalar(0);
  return derived().redux(RivetEigen::internal::scalar_sum_op<Scalar,Scalar>());
}
...
#endif
  return Scalar(derived().redux(RivetEigen::internal::scalar_sum_op<Scalar,Scalar>())) / Scalar(this->size());
#ifdef __INTEL_COMPILER
  #pragma warning pop
#endif
}
...
DenseBase<Derived>::prod() const
{
  if(SizeAtCompileTime==0 || (SizeAtCompileTime==Dynamic && size()==0))
    return Scalar(1);
  return derived().redux(RivetEigen::internal::scalar_product_op<Scalar>());
}
