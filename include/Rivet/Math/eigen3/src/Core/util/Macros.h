...
#define EIGEN_DEFAULT_MATRIX_STORAGE_ORDER_OPTION RivetEigen::RowMajor
#else
#define EIGEN_DEFAULT_MATRIX_STORAGE_ORDER_OPTION RivetEigen::ColMajor
#endif
...
//   Eval.h:91: sorry, unimplemented: inlining failed in call to 'const RivetEigen::Eval<Derived> RivetEigen::MatrixBase<Scalar, Derived>::eval() const'
...
// eigen_plain_assert is where we implement the workaround for the assert() bug in GCC <= 4.3, see bug 89
    ...
    #define eigen_plain_assert(x) \
      do { \
        if(!RivetEigen::internal::copy_bool(x)) \
          RivetEigen::internal::assert_fail(EIGEN_MAKESTRING(x), __PRETTY_FUNCTION__, __FILE__, __LINE__); \
      } while(false)
  #endif
#endif
...
#define EIGEN_UNUSED_VARIABLE(var) RivetEigen::internal::ignore_unused_variable(var);
...
// See bug 1674
    ...
    // directly for std::complex<T>, RivetEigen::half, RivetEigen::bfloat16. For these,
...
#define EIGEN_DEFAULT_IO_FORMAT RivetEigen::IOFormat(3, 0, " ", "\n", "", "")
#else
#define EIGEN_DEFAULT_IO_FORMAT RivetEigen::IOFormat()
#endif
#endif
...
#define EIGEN_GENERIC_PUBLIC_INTERFACE(Derived) \
  typedef typename RivetEigen::internal::traits<Derived>::Scalar Scalar; /*!< \brief Numeric type, e.g. float, double, int or std::complex<float>. */ \
  typedef typename RivetEigen::NumTraits<Scalar>::Real RealScalar; /*!< \brief The underlying numeric type for composed scalar types. \details In cases where Scalar is e.g. std::complex<T>, T were corresponding to RealScalar. */ \
  typedef typename Base::CoeffReturnType CoeffReturnType; /*!< \brief The return type for coefficient access. \details Depending on whether the object allows direct coefficient access (e.g. for a MatrixXd), this type is either 'const Scalar&' or simply 'Scalar' for objects that do not allow direct coefficient access. */ \
  typedef typename RivetEigen::internal::ref_selector<Derived>::type Nested; \
  typedef typename RivetEigen::internal::traits<Derived>::StorageKind StorageKind; \
  typedef typename RivetEigen::internal::traits<Derived>::StorageIndex StorageIndex; \
  enum CompileTimeTraits \
      { RowsAtCompileTime = RivetEigen::internal::traits<Derived>::RowsAtCompileTime, \
        ColsAtCompileTime = RivetEigen::internal::traits<Derived>::ColsAtCompileTime, \
        Flags = RivetEigen::internal::traits<Derived>::Flags, \
...
#define EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME,TYPEA,TYPEB) \
  (RivetEigen::internal::has_ReturnType<RivetEigen::ScalarBinaryOpTraits<TYPEA,TYPEB,EIGEN_CAT(EIGEN_CAT(RivetEigen::internal::scalar_,OPNAME),_op)<TYPEA,TYPEB>  > >::value)
