...
/** \internal
  ...
  * be created by the user using RivetEigen::fix<N> or RivetEigen::fix<N>(). In C++98-11, the former syntax does
...
/** \internal
  ...
  * value can be either RivetEigen::Dynamic (for positive quantities) or RivetEigen::DynamicIndex (for
  * quantities that can be negative).
  *
  * It is the return-type of the function RivetEigen::fix<N>(int), and most of the time this is the only
...
// Convert any integral type (e.g., short, int, unsigned int, etc.) to RivetEigen::Index
...
/** \var fix<N>()
  ...
  *
  * It is typically used in conjunction with the RivetEigen::seq and RivetEigen::seqN functions to pass compile-time values to them:
  * \code
...
/** \fn fix<N>(int)
  ...
  * such as a size or a dimension, this case is identified by RivetEigen::Dynamic, whereas runtime signed integers
  * (e.g., an increment/stride) are identified as RivetEigen::DynamicIndex. In such a case, the runtime value \a val
  ...
  * }
  * \endcode
  * In this example, the function RivetEigen::seqN knows that the second argument is expected to be a size.
  * If the passed compile-time value N equals RivetEigen::Dynamic, then the proxy object returned by fix will be dissmissed, and converted to an RivetEigen::Index of value \c n.
