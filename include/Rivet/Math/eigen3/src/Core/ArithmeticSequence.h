...
/** \class ArithmeticSequence
  ...
  *
  * It is internally used as the return type of the RivetEigen::seq and RivetEigen::seqN functions, and as the input arguments
  ...
  *
  * \sa RivetEigen::seq, RivetEigen::seqN, DenseBase::operator()(const RowIndices&, const ColIndices&), class IndexedView
  */
template<typename FirstType,typename SizeType,typename IncrType>
class ArithmeticSequence
{
...
#if EIGEN_HAS_CXX11 && ((!EIGEN_COMP_GNUC) || EIGEN_COMP_GNUC>=48)
  auto reverse() const -> decltype(RivetEigen::seqN(m_first+(m_size+fix<-1>())*m_incr,m_size,-m_incr)) {
...
-> decltype(seqN(RivetEigen::last-(size-fix<1>())*incr, size, incr))
{
  return seqN(RivetEigen::last-(size-fix<1>())*incr, size, incr);
}
...
-> decltype(seqN(RivetEigen::last+fix<1>()-size, size))
{
  return seqN(RivetEigen::last+fix<1>()-size, size);
}
#endif
...
/** \namespace RivetRivetEigen::indexing
  * \ingroup Core_Module
  * 
  * The sole purpose of this namespace is to be able to import all functions
  * and symbols that are expected to be used within operator() for indexing
  * and slicing. If you already imported the whole Eigen namespace:
  * \code using namespace RivetEigen; \endcode
  * then you are already all set. Otherwise, if you don't want/cannot import
  * the whole Eigen namespace, the following line:
  * \code using namespace RivetRivetEigen::indexing; \endcode
  * is equivalent to:
  * \code
  using RivetEigen::all;
  using RivetEigen::seq;
  using RivetEigen::seqN;
  using RivetEigen::lastN; // c++11 only
  using RivetEigen::last;
  using RivetEigen::lastp1;
  using RivetEigen::fix;
  \endcode
  */
namespace indexing {
  using RivetEigen::all;
